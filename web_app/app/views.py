...
def predict(request, ticker_value, number_of_days):
    ...
    # Extracting 'Adj Close' column as feature
    df_ml = df_ml[['Adj Close']]

    # Forecast horizon
    forecast_out = int(number_of_days)

    # Shifting 'Adj Close' column to create target variable 'Prediction'
    df_ml['Prediction'] = df_ml[['Adj Close']].shift(-forecast_out)

    # Dropping 'Prediction' column to obtain input features
    X = np.array(df_ml.drop(['Prediction'], axis=1))

    # Scaling the input features (if required)
    X = preprocessing.scale(X)

    # Creating sequences of input data and corresponding targets
    X_sequence = []
    y_sequence = []
    ...
    # Creating sequences with a length of 'sequence_length'
    for i in range(len(X) - sequence_length - forecast_out + 1):
        X_sequence.append(X[i:i+sequence_length])
        y_sequence.append(df_ml['Prediction'][i+sequence_length-1])
    ...
    split_index = int(len(X_sequence) * 0.8)  # 80% for training, 20% for testing
    ...
    num_features = 1
    clf = Sequential([
        GRU(units=64, input_shape=(sequence_length, num_features)),  # GRU layer with 64 units
 ...
 # Prediction Score
    ...
    # Create a list to store the forecasted dates and predictions
    forecast_dates = []
    forecast_predictions = []

    # Generate forecasted dates starting from today
    start_date = dt.datetime.today()
    for i in range(len(forecast)):
        forecast_dates.append(start_date + dt.timedelta(days=i))
        forecast_predictions.append(forecast[i])

    # Create a DataFrame to hold the forecasted data
    forecast_df = pd.DataFrame({"Date": forecast_dates, "Prediction": forecast_predictions})

    # Plot the forecasted data
    forecast_fig = go.Figure([go.Scatter(x=forecast_df['Date'], y=forecast_df['Prediction'])])
    forecast_fig.update_xaxes(rangeslider_visible=True)
    forecast_fig.update_layout(paper_bgcolor="#14151b", plot_bgcolor="#14151b", font_color="white")
    plot_div_forecast = plot(forecast_fig, auto_open=False, output_type='div')
    ...
    return render(request, "result.html", context={ 'plot_div': plot_div, 
                                                    ...
                                                    'plot_div_forecast': plot_div_forecast,
                                                    'Symbol': Symbol,
                                                    'Name': Name,
                                                    'Last_Sale': Last_Sale,
